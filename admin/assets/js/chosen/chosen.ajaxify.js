...
function chosen_ajaxify(id, ajax_url) {
  ...
  if (jQuery('div#' + div_id + '_chosen').hasClass('chosen-container-single')) {
    jQuery('div#' + div_id + '_chosen' + ' .chosen-search input').bindWithDelay('keyup', function(
      event,
    ) {
      // ignore arrow key
      ...
      REQUEST[id] = jQuery.ajax(
        {
          ...
          complete: function() {
            ...
            jQuery('div#' + div_id + '_chosen' + ' .active-result').each(function() {
              ...
              jQuery(this).html(
                html.replace(keyword_pattern, function(matched) {
  ...
  } else if (jQuery('div#' + div_id + '_chosen').hasClass('chosen-container-multi')) {
    // if multi
    jQuery('div#' + div_id + '_chosen' + ' input').bindWithDelay(
      'keyup',
      function(event) {
        // ignore arrow key
        if (event.keyCode >= 37 && event.keyCode <= 40) {
