(function($) {
  $(document).ready(function() {
    ...
    $('.a3rev_panel_container div.a3rev-ui-slide').each(function(i) {
      if ($(this).attr('min') != undefined && $(this).attr('max') != undefined) {
        $(this).slider({
          ...
          value: parseInt(
            $(this)
              .parent('.a3rev-ui-slide-container-end')
              .parent('.a3rev-ui-slide-container-start')
              .next('.a3rev-ui-slide-result-container')
              .children('input')
              .val(),
          ),
    ...
    $('.a3rev_panel_container input.a3rev-ui-box_shadow-enable').each(function(i) {
      if ($(this).is(':checked')) {
        $(this)
          .parent('.forminp-box_shadow')
          .find('.a3rev-ui-box_shadow-enable-container')
          .css({ visibility: 'visible', height: 'auto', overflow: 'inherit' });
      } else {
        $(this)
          .parent('.forminp-box_shadow')
          .find('.a3rev-ui-box_shadow-enable-container')
          .css({ visibility: 'hidden', height: '0px', overflow: 'hidden' });
      }
      $(this).on('a3rev-ui-onoff_checkbox-switch', function(event, value, status) {
        if (status == 'true') {
          $(this)
            .parents('.forminp-box_shadow')
            .find('.a3rev-ui-box_shadow-enable-container')
            .hide()
            .css({ visibility: 'visible', height: 'auto', overflow: 'inherit' })
            .slideDown();
    ...
    $('.a3rev_panel_container input.a3rev-ui-bg_color-enable').each(function(i) {
      if ($(this).is(':checked')) {
        $(this)
          .parent('.forminp-bg_color')
          .find('.a3rev-ui-bg_color-enable-container')
          .css({ visibility: 'visible', height: 'auto', overflow: 'inherit' });
      } else {
        $(this)
          .parent('.forminp-bg_color')
    ...
    $('.a3rev_panel_container input.a3rev-ui-onoff_checkbox').each(function(i) {
      ...
      if ($(this).attr('unchecked_label') != undefined)
        unchecked_label = $(this).attr('unchecked_label');
      ...
      if ($(this).prop('checked')) {
        $(this)
          .parents('.a3rev-ui-settings-control')
          .find('.a3rev-ui-border-corner-value-container')
          .css({ display: 'block' });
      } else {
        $(this)
          .parents('.a3rev-ui-settings-control')
      ...
      $(this).iphoneStyle({
        ...
        onChange: function(elem, value) {
          ...
          if (status == 'true') {
            ...
            elem
              .parents('.forminp-google_api_key')
              .find('.a3rev-ui-google-api-key-container')
              .slideDown();

            /* Apply for Google Map API Key */
            elem
              .parents('.forminp-google_map_api_key')
              .find('.a3rev-ui-google-api-key-container')
              .slideDown();
          } else {
            /* Apply for Border Corner */
          ...
          $('input[name="' + input_name + '"]').trigger('a3rev-ui-onoff_checkbox-switch', [
            elem.val(),
            status,
          ]);
        },
        onEnd: function(elem, value) {
          ...
          $('input[name="' + input_name + '"]').trigger('a3rev-ui-onoff_checkbox-switch-end', [
            elem.val(),
            status,
          ]);
        },
    ...
    $('.a3rev_panel_container input.a3rev-ui-onoff_radio').each(function(i) {
      ...
      $(this).iphoneStyle({
        ...
        onChange: function(elem, value) {
          ...
          if (status == 'true') {
            $('input[name="' + input_name + '"]')
              .not(current_item)
              .removeAttr('checked')
              .removeAttr('checkbox-disabled')
              .iphoneStyle('refresh');
          }
          $('input[name="' + input_name + '"]').trigger('a3rev-ui-onoff_radio-switch', [
            elem.val(),
            status,
          ]);
        },
        onEnd: function(elem, value) {
          ...
          $('input[name="' + input_name + '"]').trigger('a3rev-ui-onoff_radio-switch-end', [
            elem.val(),
            status,
          ]);
        },
    ...
    $('.a3rev_panel_container .chzn-select-deselect').chosen({
    ...
    $(document).on('click', '.a3-plugin-ui-panel-box', function() {
      ...
      if ($(this).hasClass('box_open')) {
        ...
        box_handle
          .siblings('.a3rev_panel_box_inside')
          .removeClass('box_open')
          .slideUp(500);
      } else {
        box_data.is_open = 1;
      ...
      if (
        $(this).hasClass('enable_toggle_box_save') &&
        typeof a3_admin_ui_script_params != 'undefined'
      ) {
    ...
    $(document).on('click', '.a3rev-ui-ajax_submit-button', function() {
      ...
      $.ajax({
        ...
        success: function(response) {
          data = $.parseJSON(response);
          $('#' + bt_ajax_submit.attr('id')).trigger('a3rev-ui-ajax_submit-completed', [
            bt_ajax_submit,
            data,
          ]);
        ...
        error: function(e) {
          ...
          $('#' + bt_ajax_submit.attr('id')).trigger('a3rev-ui-ajax_submit-errors', [
            e,
            bt_ajax_submit,
          ]);
    ...
    function a3rev_ui_ajax_submit(bt_ajax_submit, multi_ajax, ajax_item_id, ajax_next_item_id) {
      ...
      var current_items = $('.a3rev-ui-statistic-' + ajax_item_id)
        .find('.a3rev-ui-statistic-current-item')
        .data('current');
      ...
      if (current_items >= total_items) {
        $('#' + bt_ajax_submit.attr('id')).trigger(
          'a3rev-ui-ajax_multi_submit-' + ajax_item_id + '-completed',
          [bt_ajax_submit, multi_ajax, ajax_item_id, ajax_next_item_id],
        );
        return false;
      }
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          a3rev_ui_ajax_multi_statistic_change(
            ajax_item_id,
            new_items,
            current_items,
            total_items,
          ...
          if (typeof result.status != 'undefined' && 'completed' != result.status) {
            $('#' + bt_ajax_submit.attr('id')).trigger(
              'a3rev-ui-ajax_multi_submit-' + ajax_item_id + '-' + result.status,
              [bt_ajax_submit, multi_ajax, ajax_item_id, ajax_next_item_id],
            );
          } else {
            progressing_text.hide();
        ...
        error: function(e) {
          ...
          // Allow trigger error
          $('#' + bt_ajax_submit.attr('id')).trigger(
            'a3rev-ui-ajax_multi_submit-' + ajax_item_id + '-errors',
            [e, bt_ajax_submit, multi_ajax, ajax_item_id, ajax_next_item_id],
          );

          // Stop ajax call here
          $('#' + bt_ajax_submit.attr('id')).trigger('a3rev-ui-ajax_multi_submit-errors', [
            bt_ajax_submit,
            multi_ajax,
            ajax_item_id,
            ajax_next_item_id,
    ...
    function a3rev_ui_ajax_multi_statistic_circle_animation(
      ajax_item_id,
      current_point,
      total_point,
    ) {
      ...
      if (current_deg <= 180) {
        ...
        $('.a3rev-ui-statistic-' + ajax_item_id)
          .find('.a3rev-ui-pie')
          .removeClass('pie-more-50');
      } else {
        right_deg = 180;
        left_deg = current_deg;
        $('.a3rev-ui-statistic-' + ajax_item_id)
          .find('.a3rev-ui-pie')
          .addClass('pie-more-50');
      }

      $('.a3rev-ui-statistic-' + ajax_item_id)
        .find('.a3rev-ui-pie-left-side')
        .css('transform', 'rotate(' + left_deg + 'deg)');
      $('.a3rev-ui-statistic-' + ajax_item_id)
        .find('.a3rev-ui-pie-right-side')
        .css('transform', 'rotate(' + right_deg + 'deg)');
    }

    function a3rev_ui_ajax_multi_statistic_change(
      ajax_item_id,
      new_point,
      current_point,
      total_point,
      ...
      $({ current_point: current_point }).animate(
        { current_point: new_point },
        {
          ...
          step: function() {
            if (have_effect) {
              $('.a3rev-ui-statistic-' + ajax_item_id)
                .find('.a3rev-ui-statistic-current-item')
                .html(Math.round(this.current_point));
            }
            a3rev_ui_ajax_multi_statistic_circle_animation(
              ajax_item_id,
              this.current_point,
              total_point,
            );
    ...
    $('.a3rev_panel_container .a3rev-ui-ajax_multi_submit-button').each(function() {
      ...
      $.each(multi_ajax_registered, function(i, ajax_item) {
        ...
        $(document).on(
          'a3rev-ui-ajax_multi_submit-' + ajax_item_id + '-start',
          '#' + bt_ajax_submit.attr('id'),
          function(event, bt_ajax_submit, multi_ajax, ajax_item_id, ajax_next_item_id) {
        ...
        $(document).on(
          'a3rev-ui-ajax_multi_submit-' + ajax_item_id + '-continue',
          '#' + bt_ajax_submit.attr('id'),
          function(event, bt_ajax_submit, multi_ajax, ajax_item_id, ajax_next_item_id) {
        ...
        $(document).on(
          ...
          function(event, bt_ajax_submit, multi_ajax, ajax_item_id, ajax_next_item_id) {
            ...
            if ('' == ajax_next_item_id) {
              $('#' + bt_ajax_submit.attr('id')).trigger('a3rev-ui-ajax_multi_submit-end', [
                bt_ajax_submit,
                multi_ajax,
              ]);
              return false;
            ...
            $('#' + bt_ajax_submit.attr('id')).trigger(
              'a3rev-ui-ajax_multi_submit-' + ajax_next_item_id + '-start',
              [bt_ajax_submit, multi_ajax, ajax_next_item_id, new_ajax_next_item_id],
            );
          },
        );
      });

      $(document).on('a3rev-ui-ajax_multi_submit-end', '#' + bt_ajax_submit.attr('id'), function(
        event,
        bt_ajax_submit,
        multi_ajax,
      ) {
    ...
    $(document).on('click', '.a3rev-ui-ajax_multi_submit-button', function() {
      ...
      // Plugin have use this control type need to get this trigger to make action from plugin
      $('#' + bt_ajax_submit.attr('id')).trigger('a3rev-ui-ajax_multi_submit-click', [
        bt_ajax_submit,
      ]);
      ...
      $('#' + bt_ajax_submit.attr('id')).trigger('a3rev-ui-ajax_multi_submit-start', [
        bt_ajax_submit,
        multi_ajax,
      ]);
